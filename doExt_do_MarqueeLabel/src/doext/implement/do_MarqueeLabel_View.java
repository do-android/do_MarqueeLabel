...
import doext.define.do_MarqueeLabel_IMethod;
import doext.define.do_MarqueeLabel_MAbstract;
import doext.ui.DoMarqueeTextView;

/**
 * 自定义扩展UIView组件实现类，此类必须继承相应VIEW类，并实现DoIUIModuleView,do_MarqueeLabel_IMethod接口；
 * #如何调用组件自定义事件？可以通过如下方法触发事件：
 * this.model.getEventCenter().fireEvent(_messageName, jsonResult);
 * 参数解释：@_messageName字符串事件名称，@jsonResult传递事件参数对象；
 * 获取DoInvokeResult对象方式new DoInvokeResult(this.model.getUniqueKey());
 */
public class do_MarqueeLabel_View extends DoMarqueeTextView implements DoIUIModuleView,do_MarqueeLabel_IMethod{
	
	/**
	 * 每个UIview都会引用一个具体的model实例；
	 */
	private do_MarqueeLabel_MAbstract model;

	public do_MarqueeLabel_View(Context context) {
		super(context);
	}
	...
	public void loadView(DoUIModule _doUIModule) throws Exception {
		this.model = (do_MarqueeLabel_MAbstract)_doUIModule;
	...
	public void onPropertiesChanged(Map<String, String> _changedValues) {
		...
		if (_changedValues.containsKey("direction")) {
			setDirection(_changedValues.get("direction"));
		}
		if (_changedValues.containsKey("text")) {
			setTextLength(_changedValues.get("text"));
		}
	}
